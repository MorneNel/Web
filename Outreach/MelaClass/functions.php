...
function stringToDateTime ($string,$format = 1) {
    ...
    // For historical reasons to do with different time formats on different servers,
    // this also supports feeding the date in YYYY
    // If data fed in is YYYY, just spit it out immediately
    $splitString = explode(' ',$string);
    if (!isset($splitString[1])) return $splitString[0];
    if (strlen($splitString[0]) == 4) return $string;
